...
Game_Party.prototype.scopeFight = function(num, sneak) {
	...
	if(actor){
	point += actor.param(5);
	point += actor.param(6)/3;
	if(!sneak && actor.isStateAffected(225))point *= 2
	if(sneak && actor.isStateAffected(49))point *= 3
	if(sneak && actor.isStateAffected(90))point *= 2
	if(sneak && actor.isStateAffected(49) && actor.isLearnedSkill(361))point *= 2
	}
	}
	point /= n;
	point /= 10;
	...
	if($gameSwitches._data[9]&&$gameSwitches._data[6])es += 1;
	point-=es;
	return point;
};
...
Game_Party.prototype.enemyVisible = function(num) {
	...
	console.log(x+";"+point)
	if(x==0)return true;
	if(x==1&&point>=0.5)return true;
	if(x==2&&point>=1)return true;
	if(x==3&&point>=1.5)return true;
	if(x==4&&point>=2)return true;
	if(x==5&&point>=2.5)return true;
	if(x==6&&point>=3)return true;
	}
	return false;
};
...
Game_Party.prototype.someSneaking = function(num) {
	...
	if(y < 0)y*=-1;
	x+=y;
	if(x>=0&&point>=6)return true;
	if(x>=1&&point>=5)return true;
	if(x>=2&&point>=4)return true;
	if(x>=3&&point>=3)return true;
	if(x>=4&&point>=2)return true;
	if(x>=5&&point>=1)return true;
	if(x>=6)return true;
	return false;
};
