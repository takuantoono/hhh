...
Game_Interpreter.prototype.hostileNego = function() {
$gameTroop.members().forEach(function(enemy) {
if(!enemy.isStateAffected(311)){
var actor = $gameActors.actor($gameVariables._data[421]);
var nego = 0;
var oenemy = $dataEnemies[enemy._enemyId];
if(oenemy.meta.hostile_race && oenemy.meta.hostile_race.indexOf(this.raceId(actor))>=0)nego+=10;
if(oenemy.meta.hostile_job && oenemy.meta.hostile_job.indexOf(actor._classId)>=0)nego+=10;
if(oenemy.meta.hostile_god && oenemy.meta.hostile_god.indexOf(this.godId(actor))>=0)nego+=10;
if(Math.floor(oenemy.meta.hostile_pos)>=0)nego+=Math.floor(oenemy.meta.hostile_pos);
if(Math.floor(oenemy.meta.hostile_level)>=actor.level)nego+=20;
if($gameTroop.turnCount()>10)nego+=10;
if($gameTroop.turnCount()>20)nego+=10;
if(enemy.hp<oenemy.params[0]/2)nego+=10;
if(Math.floor( Math.random() * 101 )<nego){
if(enemy.isStateAffected(310)){
enemy.addState(311);
if(BattleManager._logWindow !== null){
  ...
  BattleManager._logWindow.push('addText', oenemy.name + 'は' + actor._name + 'の説得に応じた。'); // その下に、新しい行を表示する。
}
}else{
enemy.addState(310);
if(BattleManager._logWindow !== null){
  BattleManager._logWindow.push('popBaseLine');  // 既に表示されている行を取ってくる。 
  BattleManager._logWindow.push('pushBaseLine'); // 既に表示されている行を表示する。
  BattleManager._logWindow.push('addText', oenemy.name + 'は' + actor._name + 'の説得に興味を示した。'); // その下に、新しい行を表示する。
}
};
};
};
